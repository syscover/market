        <li{!! Miscellaneous::setCurrentOpenPage(['market-product','market-category']) !!}>
            ...
            <ul class="sub-menu">
                @if(session('userAcl')->isAllowed(Auth::user()->profile_010, 'market-product', 'access'))
                    <li{{ Miscellaneous::setCurrentPage('market-product') }}><a href="{{ route('MarketProduct') }}"><i class="fa fa-cube"></i>{{ trans_choice('market::pulsar.product', 2) }}</a></li>
                @endif
                @if(session('userAcl')->isAllowed(Auth::user()->profile_010, 'market-product', 'access'))
                    <li{{ Miscellaneous::setCurrentPage('market-product') }}><a href="#"><i class="icomoon-icon-truck"></i>Productos</a></li>
                @endif
                <li{!! Miscellaneous::setCurrentOpenPage(['market-customer-tax','market-product-tax']) !!}>
                    ...
                    <ul class="sub-menu">
                        @if(session('userAcl')->isAllowed(Auth::user()->profile_010, 'market-product-tax', 'access'))
                            <li{!! Miscellaneous::setCurrentPage('market-product-tax') !!}><a href="{{ route('MarketplaceCustomerTax') }}"><i class="a fa-users"></i>{{ trans_choice('marketplace::pulsar.customer_tax', 2) }}</a></li>
                        @endif
                    </ul>
                </li>
            </ul>
