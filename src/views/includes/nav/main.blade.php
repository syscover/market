        ...
        <li{!! is_current_resource(['market-product','market-category'], true) !!}>
            ...
            <ul class="sub-menu">
                @if(is_allowed('market-product', 'access'))
                    <li{!! is_current_resource('market-product') !!}><a href="{{ route('marketProduct', [base_lang2()->id_001]) }}"><i class="fa fa-barcode"></i>{{ trans_choice('market::pulsar.product', 2) }}</a></li>
                @endif
                @if(is_allowed('market-category', 'access'))
                    <li{!! is_current_resource('market-category') !!}><a href="{{ route('marketCategory', [base_lang2()->id_001]) }}"><i class="fa fa-cubes"></i>{{ trans_choice('market::pulsar.category', 2) }}</a></li>
                @endif
            </ul>
        </li>
        <li{!! is_current_resource(['market-cart-price-rule'], true) !!}>
            ...
            <ul class="sub-menu">
                @if(is_allowed('market-cart-price-rule', 'access'))
                    <li{!! is_current_resource('market-cart-price-rule') !!}><a href="{{ route('cartPriceRule', [base_lang2()->id_001]) }}"><i class="fa fa-shopping-cart"></i>{{ trans_choice('market::pulsar.cart_price_rule', 2) }}</a></li>
                @endif
            </ul>
        </li>
        ...
        <li{!! is_current_resource(['market-payment-method','market-order-status'], true) !!}>
            ...
            <ul class="sub-menu">
                @if(is_allowed('market-payment-method', 'access'))
                    <li{!! is_current_resource('market-payment-method') !!}><a href="{{ route('marketPaymentMethod', [base_lang2()->id_001]) }}"><i class="fa fa-random"></i>{{ trans_choice('market::pulsar.payment_method', 2) }}</a></li>
                @endif
                @if(is_allowed('market-order-status', 'access'))
                    <li{!! is_current_resource('market-order-status') !!}><a href="{{ route('marketOrderStatus', [base_lang2()->id_001]) }}"><i class="fa fa-refresh"></i>{{ trans_choice('market::pulsar.order_status', 2) }}</a></li>
                @endif
            </ul>
        </li>
    </ul>
