	...
	public function up()
	{
		if(! Schema::hasTable('012_102_group_customer_class_tax'))
		{
			Schema::create('012_102_group_customer_class_tax', function (Blueprint $table) {
				...
				$table->foreign('group_id_102', 'fk01_group_customer_class_tax')
					->references('id_300')
					->on('009_300_group')
					->onDelete('cascade')
					->onUpdate('cascade');
				$table->foreign('customer_class_tax_id_102', 'fk02_group_customer_class_tax')
					->references('id_100')
					->on('012_100_customer_class_tax')
					->onDelete('cascade')
					->onUpdate('cascade');
