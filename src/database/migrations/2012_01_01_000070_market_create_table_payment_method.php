	...
	public function up()
	{
		if (! Schema::hasTable('012_115_payment_method'))
		{
			Schema::create('012_115_payment_method', function (Blueprint $table) {
				...
				$table->foreign('lang_id_115', 'fk01_012_115_payment_method')
					->references('id_001')
					->on('001_001_lang')
					->onDelete('restrict')
					->onUpdate('cascade');
				$table->foreign('order_status_successful_id_115', 'fk02_012_115_payment_method')
					->references('id_114')
					->on('012_114_order_status')
					->onDelete('restrict')
					->onUpdate('cascade');

				$table->primary(['id_115', 'lang_id_115'], 'pk01_012_115_payment_method');
			});
		}
	}
